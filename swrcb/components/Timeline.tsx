...
import { Clock, Train, User, Play, Pause, MapPin, Calendar, ChevronDown, ChevronRight } from "lucide-react"
...
export const Timeline = ({ stations, journey, crew }) => {
  ...
  const [hasPerformedInitialStart, setHasPerformedInitialStart] = useState(false)
  ...
  const adjustedStations = stations.map((station) => ({
    ...station,
    arrivalTime: totalDelay ? addMinutesToTime(station.arrivalTime, totalDelay) : station.arrivalTime,
    departureTime: totalDelay ? addMinutesToTime(station.departureTime, totalDelay) : station.departureTime,
  }))
  ...
  const createTimelineEvents = () => {
    ...
    const interruptionEvents = interruptions
      ? interruptions?.flatMap((interruption) => {
          ...
          const endTimeStr = interruption.end_time ? moment(interruption.end_time).format("HH:mm") : null
    ...
    const stationEvents = groupedStations
      ? Object.entries(groupedStations).flatMap(([timeSegment, segmentStations]) =>
    ...
    const allEvents = [...interruptionEvents, ...stationEvents].sort((a, b) => a.normalizedTime - b.normalizedTime)
  ...
  useEffect(() => {
    ...
    const updateData = () => {
      ...
      const expected = stations.find((station) => {
        ...
        const nextStationTime = nextStation ? getTimeInMinutes(nextStation.arrivalTime) : Number.POSITIVE_INFINITY
      ...
      if (expected) {
        const stationIndex = stations.findIndex((s) => s.code === expected.code)
        ...
        if (nextStation) {
          ...
          if (currentTimeMinutes >= nextStationTime && !reachedStations.has(nextStation.code)) {
  ...
  }, [stations, reachedStations, recordCurrentStation, recordNextStation, recordStationReached])
  ...
  useEffect(() => {
    if (stations && expectedStation && !hasStartedJourney && !hasInitializedJourney && !hasPerformedInitialStart) {
      startNewJourney(expectedStation)
      setHasStartedJourney(true)
      setHasInitializedJourney(true)
      setHasPerformedInitialStart(true)
    }
  }, [expectedStation, stations, startNewJourney, hasStartedJourney, hasInitializedJourney, hasPerformedInitialStart])
            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <div className="flex items-center mt-2 text-transnet-gray">
                  ...
                  <p>{expectedStation ? `Currently at: ${expectedStation.name}` : "Journey not started"}</p>
                </div>
              </div>
            </div>
                            ...
                            <div className="space-y-2">
                              <div className="flex items-center space-x-4">
                                <h3 className="font-semibold text-lg">
                                  {event.category === "station" ? event.station.name : "Train Interruption"}
                                </h3>
                                <span
                              ...
                              {event.category === "interruption" && (
                                <div className="space-y-1">
                                  ...
                                  {event.interruption.location_description && (
                                    <p className="text-sm text-gray-600">
                                      Location: {event.interruption.location_description}
                                    </p>
                                  )}
                                  {getInterruptionStatusBadge(event.interruption)}
                                </div>
                              )}
                            </div>
                            <div>
                              <span
                                className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium ${
                                  isEventPast ? "bg-transnet-gray text-white" : "bg-transnet-green text-white"
                                }`}
                              >
